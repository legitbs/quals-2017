...
uint8_t *CStreamData::GetCopyData( void )
{
	uint8_t *pCopyData = new uint8_t[GetLength()];
	...
	uint32_t writeRemaining = GetLength();
	uint32_t sourcePos = 0;
	...
	while ( writeRemaining > 0 )
	{
		...
		// Now copy in write amount
		memcpy( m_memoryChunks[chunkIndex] + chunkPos, (uint8_t*)pCopyData + sourcePos, writeAmount );
