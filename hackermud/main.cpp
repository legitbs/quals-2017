...
bool CheckNewUser( CServerConnection &oConnection )
{
	...
	{
		printf( "%sEnter new username: ", FC_BRIGHT_CYAN );
...
bool GetProofOfWork( CServerConnection &oConnection )
{
	...
	if ( oConnection.RecvCommand( cmdNumber, pCmdData, cmdLen ) )
	{
		...
		uint8_t POW_K = pCmdData[0];
		uint8_t POW_N = pCmdData[1];
	
		uint32_t seed[4];
		seed[0] = *((uint32_t*)(pCmdData+2));
		seed[1] = *((uint32_t*)(pCmdData+6));
		seed[2] = *((uint32_t*)(pCmdData+10));
		seed[3] = *((uint32_t*)(pCmdData+14));
		...
		if ( !g_pProofHash )
		{
			printf( "%sFailed to allocate Proof of Work function!%s\n", FC_NORMAL_RED, FC_NORMAL_WHITE );
			return (false);
		}
	}
	else
